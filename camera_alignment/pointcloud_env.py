...
class PointcloudEnv:
    ...
    Args:
        ...
        camera_extrinsics_dir (str): directory containing the camera parameters
    ...
    def __init__(self, 
        ...
        if camera_extrinsics_dir is None:
            curr_dir = os.path.dirname(os.path.abspath(__file__))
            camera_extrinsics_dir = os.path.join(curr_dir, 'calibration/calibration_results')
            print("path", camera_extrinsics_dir)
        camera_params_files = {
            0: 'cam0_extrinsics.npz',
            1: 'cam1_extrinsics.npz',
            # 2: 'cam2_extrinsics.npz',
            # 3: 'cam3_extrinsics.npz',
        }

        self.camera_transforms = load_camera_transforms(camera_extrinsics_dir, camera_params_files)

        # Load the default camera alignment. Load identity when set to False.
        if camera_alignments is None:
            curr_dir = os.path.dirname(os.path.abspath(__file__))
            camera_alignments = os.path.join(curr_dir, 'calibration/alignment_results/camera_alignments.npz')
    ...
    def get_single_pcd(self, cam_id, color=False):
        ...
        if pcd is not None:
            ...
            pcd_down_mask = np.random.choice(pcd_size, int(pcd_size / 2))
            pcd_down = pcd.select_by_index(pcd_down_mask)

            # Voxel downsample
            voxel_size = 0.005  # For initial processing we use 0.5cm  # original 0.005
            pcd_down = pcd_down.voxel_down_sample(voxel_size)

            # Remove outliers
            radius = 0.02 # original 0.02
            ...
            # --
            R = pcd_down.get_rotation_matrix_from_xyz((-0.025 * np.pi, 0.01 * np.pi, 0.01 * np.pi))
            pcd_down = pcd_down.rotate(R, center=(0,0,0))
            # --
            
            return pcd # pcd_down

            return self.transform_raw_pcd(pcd, cam_id)
        else:
            return None
