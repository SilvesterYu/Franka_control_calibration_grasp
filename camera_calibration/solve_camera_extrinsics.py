...
def estimate_tag_pose(finger_pose):
    ...
    print("tag to hand", tag_to_hand)
    print("R_tag_to_hand", R_tag_to_hand.as_matrix())
    tag_to_hand_inv = np.linalg.inv(tag_to_hand)
    print("inverse of tag to hand", tag_to_hand_inv)
...
if __name__ == "__main__":
    ...
    # check the printed "reprojection error" for calivration quality
    T = solve_extrinsic(gripper_poses, target_poses_in_camera)
    # Save the result
    calib_dirname = os.path.join(current_dir, "calibration_results")
    os.makedirs(calib_dirname, exist_ok=True)
    filepath = os.path.join(calib_dirname, f"cam{cam_id}_extrinsics.npz")
